<div id="kanban-wrapper">
    ...
    foreach ($tasks as $task) {
        ...
        if ($login_user->user_type == "staff" && $can_edit_tasks && $supplier_id) {
        ...
        $item = $exising_items . modal_anchor(get_uri("projects/supplier_task_view"), "<span class='avatar'>" .
                        ...
                        $task_labels . $task_checklist_status, array("class" => "kanban-item d-block $disable_dragging $unread_comments_class", "data-id" => $task->id, "data-supplier_id" => $task->supplier_id, "data-sort" => $task->new_sort, "data-post-id" => $task->id, "title" => app_lang('task_info') . " #$task->id", "data-modal-lg" => "1"));
...
<script type="text/javascript">
    ...
    saveStatusAndSort = function ($item, status) {
        ...
        var $prev = $item.prev(),
                ...
                supplier_id = $item.attr("data-supplier_id");
        ...
        $.ajax({
            ...
            type: "POST",
            data: {id: id, sort: newSort, status_id: status, project_id: supplier_id},
            success: function () {
                appLoader.hide();
