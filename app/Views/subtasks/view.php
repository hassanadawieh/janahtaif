            ...
            <div class="col-md-12">
                <div class="card">
                    <div class="page-title clearfix">
                        <h1><?php echo app_lang("task_info") . " #$model_info->sub_task_id"; ?></h1>

                        <?php if($model_info->main_task_status==1){
                            if(!$is_supplier){
                            if ($can_edit_tasks) { ?>
                            <div class="title-button-group">
                                <span class="dropdown inline-block">
                                    ...
                                    <ul class="dropdown-menu float-end" role="menu">
                                        <li role="presentation"><?php echo modal_anchor(get_uri("subtasks/task_modal_form"), "<i data-feather='edit-2' class='icon-16'></i> " . app_lang('edit_task'), array("title" => app_lang('edit_task'), "data-post-id" => $model_info->id, "data-post-mang" => $mang, "data-post-view_type" => "details", "id" => "task-details-edit-btn", "class" => "dropdown-item")); ?></li>
...
<?php } else { ?>
    ...
    <script type="text/javascript">
        $(document).ready(function () {
            ...
            var browserState = {Url: "<?php echo get_uri("subtasks/task_view/" . $model_info->id); ?>"};
    ...
    <div class="modal-footer">
        ...
        if ($can_edit_tasks) {
            ...
            echo modal_anchor(get_uri("subtasks/task_modal_form"), "<i data-feather='edit-2' class='icon-16'></i> " . app_lang('edit_task'), array("class" => "btn btn-default", "data-post-id" => $model_info->id, "data-post-mang" => $mang, "title" => app_lang('reserv_mang').'-'.app_lang('sub_task'). " #$model_info->sub_task_id"));
        }
    }
    }
        ?>
...
<?php
$task_link = anchor(get_uri("subtasks/task_view/$model_info->id"), '<i data-feather="external-link" class="icon-16 task-link-btn"></i>', array("target" => "_blank", "class" => "p15"));
?>
