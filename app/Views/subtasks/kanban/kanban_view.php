<div id="kanban-wrapper">
    ...
    foreach ($tasks as $task) {
        ...
        $start_date = "";
        if ($task->act_return_date) {
            $act_return_date = "<div class='mt10 font-12 float-start' title='" . app_lang("start_date") . "'><i data-feather='calendar' class='icon-14 text-off mr5'></i> " . format_to_date($task->act_return_date, false) . "</div>";
        }
       ...
       $act_return_date = "";
        ...
        $tmp_return_date = "";
        if ($task->tmp_return_date && $task->tmp_return_date!="0000-00-00") {
            $tmp_return_date = "<div class='mt10 font-12 float-end' title='" . app_lang("tmp_return_date") . "'><i data-feather='calendar' class='icon-14 text-off mr5'></i> " . format_to_date($task->tmp_return_date, false) . "</div>";
        }
        ...
        $item =  $exising_items .modal_anchor(get_uri("subtasks/task_view"), "<span class='avatar'>" .
                        "<img src='" . get_avatar($task->assigned_to_avatar) . "'>" .
                        "</span>" . $task->sub_task_id.' '. $task->guest_nm  . "<div class='clearfix'>" . $act_return_date . $tmp_return_date . "</div>". $supplier_name . $kanban_custom_fields_data .
                        $task_labels , array("class" => "kanban-item d-block $disable_dragging $unread_comments_class","data-sort" => $task->new_sort, "data-id" => $task->id, "data-project_id" => $task->pnt_task_id, "data-post-id" => $task->id, "data-post-mang" => "reservmang", "title" => app_lang("reserv_mang").' - '.app_lang('task_info') . " #$task->sub_task_id", "data-modal-lg" => "1"));
...
<script type="text/javascript">
    ...
    saveStatusAndSort = function ($item, status) {
        ...
        $.ajax({
            ...
            type: "POST",
            data: {id: id, sort: newSort, status_id: status,mang:'reservmang'},
