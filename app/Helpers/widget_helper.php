...
if (!function_exists('my_open_tasks_widget')) {
    ...
    function my_open_tasks_widget() {
        $ci = new Security_Controller(false);
        $view_data["totals"] = $ci->Tasks_model->count_my_open_tasks($ci->login_user->id);
        $template = new Template();
...
if (!function_exists('my_task_stataus_widget')) {
    ...
    function my_task_stataus_widget($custom_class = "") {
        ...
        $view_data["task_statuses"] = $ci->Sub_tasks_model->get_task_statistics(array("user_id" => $ci->login_user->id,"selected_year" => $ci->session->get('selected_year')))->task_statuses;
        $view_data["custom_class"] = $custom_class;

        $template = new Template();
        return $template->view("subtasks/my_task_status_widget", $view_data);
    }
...
if (!function_exists('my_tasks_list_widget')) {
    ...
    function my_tasks_list_widget() {
        ...
        } else if (get_array_value($ci->login_user->permissions, "can_show_tasks") == "1") {
            return $template->view("tasks/my_tasks_list_widget", $view_data);
        }
...
if (!function_exists('my_subtasks_list_24hour')) {
function my_subtasks_list_24hour(){
        $Sub_tasks_model = model("App\Models\Sub_tasks_model");
    $ci = new Security_Controller(false); 
    $template = new Template();
    $options["selected_year"] = $ci->session->get('selected_year');
        $view_data['subtasks'] = $Sub_tasks_model->get_task_befor24($options);

        return $template->view("subtasks/open_tasks_widget", $view_data);
       /* task add/edit modal */
 ...
 * @return html
 */
if (!function_exists('total_suppliers_widget')) {

    function total_suppliers_widget() {
        $Suppliers_model = model("App\Models\Suppliers_model");
        $view_data["total"] = $Suppliers_model->count_total_suppliers();
        $template = new Template();
        return $template->view("suppliers/total_suppliers_widget", $view_data);
    }

}

if (!function_exists('total_suppliers_tasks_widget')) {

    function total_suppliers_tasks_widget($supplier_id) {
        $Tasks_model = model("App\Models\Tasks_model");
        $view_data["total"] = $Tasks_model->count_supplier_tasks($supplier_id);
        $template = new Template();
        return $view_data["total"];
    }
...
/**
 * get tasks count widget
 * @param boolean $return_as_data
 * @return html
 */
if (!function_exists('tasks_count_widget')) {

    function tasks_count_widget($option="",$title="") {
        $ci = new Security_Controller(false);
        /*$Tasks_model = model("App\Models\Tasks_model");

        $view_data["no_invoice"] = $Tasks_model->count_total_info(array("no_invoice" => "312"));
        $view_data["no_christening_number"] = $Tasks_model->count_total_info(array("no_christening_number" => "312"));
...
if (!function_exists('all_tasks_overview_widget')) {
    ...
    function all_tasks_overview_widget() {
        ...
        if ($ci->login_user->is_admin || get_array_value($permissions, "can_manage_all_projects") == "1") {
            ...
            $options["mang"] = "reserv";
            $options["selected_year"] = $ci->session->get('selected_year');
        } else if (get_array_value($permissions, "reserv_mang") == "1") {
            $options["mang"] = "reserv";
            $options["selected_year"] = $ci->session->get('selected_year');
        } else {
            $options["mang"] = "supply";
            $options["selected_year"] = $ci->session->get('selected_year');
             //don't show all tasks to non-admin users
        }

        $view_data["task_statuses"] = $ci->Sub_tasks_model->get_task_statistics($options)->task_statuses;
        //$view_data["task_priorities"] = $ci->Tasks_model->get_task_statistics($options)->task_priorities;

        $template = new Template();
        return $template->view("subtasks/all_tasks_overview_widget", $view_data);
    }

}


if (!function_exists('all_tasks_overview_para_widget')) {

    function all_tasks_overview_para_widget($mang) {
        $ci = new Security_Controller(false);
        $permissions = $ci->login_user->permissions;
 ...
 * @return html
 */
if (!function_exists('main_tasks_overview_widget')) {

    function main_tasks_overview_widget() {
        $ci = new Security_Controller(false);
        $permissions = $ci->login_user->permissions;
       ...
       $options["selected_year"] = $ci->session->get('selected_year');
        ...
        $template = new Template();
        return $template->view("tasks/all_tasks_overview_widget", $view_data);
    }
...
if (!function_exists('task_filter_overview_widget')) {

    function task_filter_overview_widget($options = array()) {
        $ci = new Security_Controller(false);

        
        $all_subtasks = $ci->Sub_tasks_model->get_details()->getResult();
        

        $options["filter"] = "no_supplier";
        $options["selected_year"] = $ci->session->get('selected_year');
        $subtasks = $ci->Sub_tasks_model->get_details($options)->getResult();

        $options2["filter"] = "no_act_return_date";
        $options2["selected_year"] = $ci->session->get('selected_year');
        $subtasks2 = $ci->Sub_tasks_model->get_details($options2)->getResult();

        $options3["filter"] = "no_act_out_time";
        $options3["selected_year"] = $ci->session->get('selected_year');
        $subtasks3 = $ci->Sub_tasks_model->get_details($options3)->getResult();

        $options4["filter"] = "wait_inv";
        $options4["selected_year"] = $ci->session->get('selected_year');
        $subtasks4 = $ci->Sub_tasks_model->get_details($options4)->getResult();
        ...
        $view_data["no_supplier"] = count($subtasks);
        $view_data["no_act_return_date"] = count($subtasks2);
        $view_data["no_act_out_time"] = count($subtasks3);
        $view_data["wait_inv"] = count($subtasks4);
        $view_data["all_subtasks"] = count($all_subtasks);
        

        $template = new Template();
        return $template->view("dashboards/task_filter_overview_widget", $view_data);
    }
...
function get_years_list() {
        ...
        $all_years = $ci->Sub_tasks_model->get_years()->getResult();
        ...
        if ($all_years) {
            foreach ($all_years as $year) {
                $years_dropdown[$year->year] = $year->year;
            }
        }*/
        ...
        if ($all_years) {
            foreach ($all_years as $year) {
                if($year->year==$this_year){
                    $is_selected=true;
                    $selected_value=$year->year;
                }else{
                    $is_selected=false;
                }
                $years_dropdown[] = array("id" => $year->year, "text" => $year->year, "isChecked" => $is_selected);
                $i++;
            }
        }
        
        $view_data["years_dropdown"] = $all_years;
        $view_data["selected_value"] = $selected_value;
        $view_data["session_selected_value"] = $ci->session->get('selected_year');
        $template = new Template();

        return $template->view("settings/topbar_parts/select_year_session", $view_data);//$view_data;//$template->view("dashboards/year_select_view", $view_data);
