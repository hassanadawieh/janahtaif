...
if (!function_exists('get_notification_config')) {
    ...
    function get_notification_config($event = "", $key = "", $info_options = array()) {
        ...
        $subtask_link = function ($options) {

            $url = "";
            $ajax_url = "";
            $id = "";

            if (isset($options->sub_task_id)) {
                $ajax_url = get_uri("subtasks/task_view/");
                $id = $options->sub_task_id;
                $url = get_uri("subtasks/task_view/" . $id);
            }

            if ((isset($options->sub_task_id) && $options->sub_task_id) ) {
                return array("url" => $url, "ajax_modal_url" => $ajax_url, "large_modal" => "1", "id" => $id);
            } else {
                //return all tasks link for reminder notifications
                return array("url" => get_uri("subtasks/index"));
            }
        };
        ...
        $task_link = function ($options) {
            ...
            if (isset($options->task_id)) {
                $ajax_url = get_uri("tasks/task_view/");
                $id = $options->task_id;
                $url = get_uri("tasks/task_view/" . $id);
            }
        ...
        $events = array(
            ...
            "sub_task_created" => array(
                "notify_to" => array("project_members", "client_primary_contact", "client_all_contacts", "client_assigned_contacts", "task_assignee", "task_collaborators", "team_members", "team"),
                "info" => $subtask_link
            ),
            "sub_task_updated" => array(
                "notify_to" => array("project_members", "client_primary_contact", "client_all_contacts", "client_assigned_contacts", "task_assignee", "task_collaborators", "team_members", "team"),
                "info" => $subtask_link
            ),
