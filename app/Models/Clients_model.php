...
class Clients_model extends Crud_model {
    ...
    function get_details($options = array()) {
        ...
        if ($client_status) {
            $where .= " AND $clients_table.status='$client_status'";
        }
        ...
        $available_order_by_list = array(
            ...
            "primary_contact" => $clients_contact . ".first_name",
        ...
        $sql = "SELECT SQL_CALC_FOUND_ROWS $clients_table.*, CONCAT($clients_contact.first_name, ' ', $clients_contact.last_name) AS primary_contact, $clients_contact.id AS primary_contact_id, $clients_contact.first_name AS contact_avatar, $payment_value_select AS payment_received $select_custom_fieds,
        ...
        FROM $clients_table
        LEFT JOIN $clients_contact ON $clients_contact.client_id = $clients_table.id AND $clients_contact.deleted=0 AND $clients_contact.is_primary_contact=1 
        ...
        LEFT JOIN (SELECT $clients_contact.id, CONCAT($clients_contact.first_name, ' ', $clients_contact.last_name) AS owner_name, $clients_contact.first_name AS owner_avatar FROM $clients_contact WHERE $clients_contact.deleted=0 ) AS owner_details ON owner_details.id=$clients_table.owner_id
        $join_custom_fieds               
    ...
    function get_primary_contact($client_id = 0, $info = false) {
        ...
        $sql = "SELECT $clients_contact.id, $clients_contact.first_name, $clients_contact.last_name
        FROM $clients_contact
        WHERE $clients_contact.deleted=0 AND $clients_contact.client_id=$client_id AND $clients_contact.is_primary_contact=1";
    ...
    function update_client_status($client_id) {
        $clients_table = $this->db->prefixTable('clients');
        $update_client_sql = "UPDATE $clients_table SET $clients_table.status=2 WHERE $clients_table.id=$client_id; ";
        $this->db->query($update_client_sql);
        return true;
    ...
    function delete_client_and_sub_items($client_id) {
        ...
        //$tasks_table_list_sql = "SELECT id FROM $tasks_table WHERE $tasks_table.deleted=0 AND $tasks_table.client_id=$client_id; ";
        //$tasks_list = $this->db->query($tasks_table_list_sql)->getResult();


        //$delete_task_sql = "UPDATE $tasks_table SET $tasks_table.deleted=1 WHERE $tasks_table.client_id=$client_id; ";
        //$this->db->query($delete_task_sql);

        $delete_projects_sql = "UPDATE $projects SET $projects.deleted=1 WHERE $projects.client_id=$client_id; ";
        $this->db->query($delete_projects_sql);

        $delete_contacts_sql = "UPDATE $clients_contact SET $clients_contact.deleted=1 WHERE $clients_contact.client_id=$client_id; ";
        ...
        /*foreach ($tasks_list as $tasks_r) {
            $delete_sub_tasks_sql = "UPDATE $sub_tasks_table SET $sub_tasks_table.deleted=1 WHERE $sub_tasks_table.pnt_task_id=$tasks_r->id; ";
        $this->db->query($delete_sub_tasks_sql);
