...
class Notifications_model extends Crud_model {
    ...
    function create_notification($event, $user_id, $options = array()) {
        ...
        $sub_task_id = get_array_value($options, "sub_task_id");
        ...
        if (in_array("task_assignee", $notify_to_terms) && $task_id) {
            $where .= " OR ($users_table.id=(SELECT $tasks_table.created_by FROM $tasks_table WHERE $tasks_table.id=$task_id)) ";
        }

         if (in_array("task_assignee", $notify_to_terms) && $sub_task_id) {
            $where .= " OR ($users_table.id=(SELECT $sub_tasks_table.created_by FROM $sub_tasks_table WHERE $sub_tasks_table.id=$sub_task_id)) ";
        }
        ...
        $sub_task_info = NULL;
        if ($sub_task_id) {
            $sub_task_info = $this->db->query("SELECT $sub_tasks_table.* FROM $sub_tasks_table WHERE $sub_tasks_table.id=$sub_task_id ")->getRow();
        }
        ...
        if ($notify_to->resultID->num_rows) {
            foreach ($notify_to->getResult() as $user) {
                ...
                if ($sub_task_info && !$this->notify_to_this_user_for_this_sub_task($sub_task_info, $user)) {
                    continue; //skip next lines for this loop
                }
        ...
        $data = array(
            ...
            "sub_task_id" => $sub_task_id ? $sub_task_id : "",
    ...
    private function notify_to_this_user_for_this_sub_task($task_info, $user) {
        if ($user->user_type === "staff") {
            //check if user has restriction to view all tasks
            if ($user->permissions) {
                $permissions = unserialize($user->permissions);
                ...
                if ($show_assigned_tasks_only) {
                    //the user has permission to access only assigned tasks
                    //$collaborators_array = explode(',', $task_info->collaborators);
                    if ($task_info->created_by != $user->id ) {
                        return false;
                    }
                }
            }
    ...
    function get_notifications($user_id, $offset = 0, $limit = 20,$year=2024) {
...
$sub_tasks_table = $this->db->prefixTable('sub_tasks');
        ...
        LEFT JOIN $sub_tasks_table ON $sub_tasks_table.id=$notifications_table.sub_task_id
        ...
        WHERE $notifications_table.deleted=0 AND YEAR($sub_tasks_table.created_at)=$year AND FIND_IN_SET($user_id, $notifications_table.notify_to) != 0
