...
class Suppliers_model extends Crud_model {
    ...
    function get_details($options = array()) {
       ...
       /* $status = $this->_get_clean_value($options, "status");
        ...
        if ($created_by) {
            $where .= " AND $suppliers_table.created_by=$created_by";
        }
        ...
        if ($show_own_clients_only_user_id) {
            $where .= " AND ($suppliers_table.created_by=$show_own_clients_only_user_id OR $suppliers_table.owner_id=$show_own_clients_only_user_id)";
        }
        ...
        if ($quick_filter) {
            $where .= $this->make_quick_filter_query($quick_filter, $suppliers_table, $projects_table, $invoices_table, $taxes_table, $invoice_payments_table, $invoice_items_table, $estimates_table, $estimate_requests_table, $tickets_table, $orders_table, $proposals_table);
        }

        $start_date = $this->_get_clean_value($options, "start_date");
        if ($start_date) {
            $where .= " AND DATE($suppliers_table.created_date)>='$start_date'";
        }
        $end_date = $this->_get_clean_value($options, "end_date");
        if ($end_date) {
            $where .= " AND DATE($suppliers_table.created_date)<='$end_date'";
        }

        $client_groups = $this->_get_clean_value($options, "client_groups");
        $where .= $this->prepare_allowed_client_groups_query($suppliers_table, $client_groups);

        //prepare custom fild binding query
        $custom_fields = get_array_value($options, "custom_fields");
        $custom_field_filter = get_array_value($options, "custom_field_filter");
        $custom_field_query_info = $this->prepare_custom_field_query_string($custom_field_type, $custom_fields, $suppliers_table, $custom_field_filter);
        ...
        $available_order_by_list = array(
            "id" => $suppliers_table . ".id",
            "name" => $suppliers_table . ".name",
            "created_date" => $suppliers_table . ".created_date",
            "address" => $suppliers_table . ".address"
        ...
        if ($search_by) {
            ...
            $where .= " AND (";
            $where .= " $suppliers_table.id LIKE '%$search_by%' ESCAPE '!' ";
            $where .= " OR $suppliers_table.name LIKE '%$search_by%' ESCAPE '!' ";
        ...
        $sql = "SELECT SQL_CALC_FOUND_ROWS $suppliers_table.*

        FROM $suppliers_table
        
        WHERE $suppliers_table.deleted=0 $where $custom_fields_where  
        $order $limit_offset";
    ...
    function delete_supplier_and_sub_items($supplier_id) {
        ...
        //delete client
        $delete_client_sql = "UPDATE $suppliers_table SET $suppliers_table.deleted=1 WHERE $suppliers_table.id=$supplier_id; ";
    ...
    function count_total_suppliers($options = array()) {
        $suppliers_table = $this->db->prefixTable('suppliers');
        $tickets_table = $this->db->prefixTable('tickets');
        $invoices_table = $this->db->prefixTable('invoices');
        $invoice_payments_table = $this->db->prefixTable('invoice_payments');
        ...
        if ($filter) {
            $where .= $this->make_quick_filter_query($filter, $suppliers_table, $projects_table, $invoices_table, $taxes_table, $invoice_payments_table, $invoice_items_table, $estimates_table, $estimate_requests_table, $tickets_table, $orders_table, $proposals_table);
        }

        

        $sql = "SELECT COUNT($suppliers_table.id) AS total
        FROM $suppliers_table 
        WHERE $suppliers_table.deleted=0 AND $suppliers_table.is_lead=0 $where";
