...
class Security_Controller extends App_Controller {
    ...
    public function __construct($redirect = true) {
        ...
        if($session->get('selected_year')==null){
            $y=date("Y");
            $session->set('selected_year', $y);
    ...
    protected function can_create_tasks($in_project = true) {
        if ($this->login_user->user_type == "staff") {
            if ($this->can_manage_all_projects()) {
                ...
                //check is user a project member
                if ($in_project) {
                    return true; //check the specific project permission
                } else {
                    return true;
                }
            }
    ...
    protected function can_create_subtasks($in_project = true) {
            if ($this->can_manage_all_projects()) {
                return true;
            } else if (get_array_value($this->login_user->permissions, "can_create_subtasks") == "1") {
                //check is user a project member
    ...
    protected function can_edit_subtasks_after_closed($in_project = true) {
            if ($this->can_manage_all_projects()) {
                return true;
            } else if (get_array_value($this->login_user->permissions, "can_edit_subtasks_after_closed") == "1") {
                //check is user a project member
    ...
    protected function can_edit_subtasks_after_review($in_project = true) {
            if ($this->can_manage_all_projects()) {
                return true;
            } else if (get_array_value($this->login_user->permissions, "can_edit_subtasks_after_review") == "1") {
                //check is user a project member
    ...
    protected function can_show_subtasks_report($in_project = true) {
            if ($this->login_user->is_admin) {
                return true;
            } else if (get_array_value($this->login_user->permissions, "can_show_subtasks_report") == "1") {
                //check is user a project member
    ...
    protected function can_edit_subtasksold() {
            if ($this->can_manage_all_projects()) {
                return true;
            } else if (get_array_value($this->login_user->permissions, "can_edit_subtasks") == "1") {
                //check is user a project member
                return true;
            }
        }

    protected function can_edit_subtasks($sub_tasks_id = 0) {
            if ($this->can_manage_all_projects()) {
                return true;
            }

            $can_edit_subtasks = get_array_value($this->login_user->permissions, "can_edit_subtasks");
            $can_edit_only_own_subtasks = get_array_value($this->login_user->permissions, "can_edit_only_own_subtasks");

            if ($can_edit_subtasks) {
                return true;
            }

            if ($sub_tasks_id) {
                $project_info = $this->Sub_tasks_model->get_one($sub_tasks_id);
                if ($can_edit_only_own_subtasks && $project_info->created_by === $this->login_user->id) {
                    return true;
                }
            } else if ($can_edit_only_own_subtasks) { //no project given and the user has partial access
                return true;
            }
        
    }

        protected function is_supply_mang() {
        if ($this->login_user->user_type == "staff") {
            if ($this->can_manage_all_projects()) {
                return true;
            } else if (get_array_value($this->login_user->permissions, "supply_mang") == "1") {
                //check is user a project member
                return true;
            }
        } 
    ...
    protected function is_reserv_mang() {
        if ($this->login_user->user_type == "staff") {
            if ($this->can_manage_all_projects()) {
                return true;
            } else if (get_array_value($this->login_user->permissions, "reserv_mang") == "1") {
                //check is user a project member
                return true;
            }
        } 
    ...
    protected function show_own_suppliers_only_user_id() {
        if ($this->login_user->user_type === "staff") {
            return get_array_value($this->login_user->permissions, "supplier") == "own" ? $this->login_user->id : false;
        }
    }
    ...
    protected function _get_projects_client_dropdown($client_id) {
        $project_options = array("status" => "open");
        //if ($this->login_user->user_type == "staff") {
            //if (!$this->can_manage_all_projects()) {
                $project_options["client_id"] = $client_id; //normal user's should be able to see only the projects where they are added as a team mmeber.
    ...
    protected function _get_suppliers_dropdown() {
        $supplier_options = array("deleted" => "0");
        if ($this->login_user->user_type == "staff") {
            if (!$this->can_manage_all_projects()) {
                $project_options["user_id"] = $this->login_user->id; //normal user's should be able to see only the projects where they are added as a team mmeber.
            }
        ...
        $suppliers = $this->Suppliers_model->get_details($supplier_options)->getResult();
        $suppliers_dropdown = array("" => "-");

        if ($suppliers) {
            foreach ($suppliers as $supplier) {
                $suppliers_dropdown[$supplier->id] = $supplier->name;
            }
        }

        return $suppliers_dropdown;
    }
    // hassan_driver adawiyeh
     protected function _get_drivers_dropdown() {
        $driver_options = array("deleted" => "0" , "status" => "1", "offDriversStatus" => "3");
        // $driver_options = array("deleted" => "0" , "status" => "1" , "status" => "3");
        if ($this->login_user->user_type == "staff") {
            if (!$this->can_manage_all_projects()) {
                $project_options["user_id"] = $this->login_user->id; //normal user's should be able to see only the projects where they are added as a team mmeber.
            }
        ...
        if($drivers) {
            foreach ($drivers as $driver) {
                $drivers_statuses[$driver->id] = $driver->status;
            }
        }
        ////////////////////
        // if ($drivers) {
        ...
        return $drivers_data = [$drivers_dropdown , $drivers_statuses];
    }
    ...
    protected function _get_drivers_dropdown2($driver_id) {
        ...
        if($driver_id!=0){
            if(!$found){
                $drivers_dropdown[$driver_id] = "DeletedOrDisabled";
            }
        }
        if($drivers) {
            foreach ($drivers as $driver) {
    ...
    protected function _get_cars_type_dropdown() {
        $cars_type_options = array("deleted" => "0","status" => "1");
        if ($this->login_user->user_type == "staff") {
            if (!$this->can_manage_all_projects()) {
                $project_options["user_id"] = $this->login_user->id; //normal user's should be able to see only the projects where they are added as a team mmeber.
            }
        ...
        $cars_type = $this->Cars_type_model->get_details($cars_type_options)->getResult();
        $cars_type_dropdown = array("" => "-".app_lang("car_type")."-");

        if ($cars_type) {
            foreach ($cars_type as $car_type) {
                $cars_type_dropdown[$car_type->id] = $car_type->car_type;
            }
        }

        return $cars_type_dropdown;
    }
    protected function _get_cars_type_dropdown2($car_type_id) {
        $cars_type_options = array("deleted" => "0","status" => "1");
        if ($this->login_user->user_type == "staff") {
            if (!$this->can_manage_all_projects()) {
                $project_options["user_id"] = $this->login_user->id; //normal user's should be able to see only the projects where they are added as a team mmeber.
        ...
        if($car_type_id!=0){
            if(!$found){
                $cars_type_dropdown[$car_type_id] = "DeletedOrDisabled";
            }
        }
    ...
    //get tasks dropdown
    protected function _get_tasks_dropdown() {
        if($this->can_update_maintask_after_closed()){
        $tasks_options = array("deleted" => "0","client_status" => "1","selected_year" => $this->session->get('selected_year'));
    }else{
        $tasks_options = array("deleted" => "0","client_status" => "1","status_ids" => "1","selected_year" => $this->session->get('selected_year'));
    }
        ...
        if ($tasks) {
            foreach ($tasks as $task) {
                ...
                $tasks_dropdown[$task->id] = '('.$task->id.')'.$task->company_name.' - '.$project_title;
            }
        }
    ...
    protected function _get_cities_dropdown() {
        $city_options = array("deleted" => "0");
        

        $cities = $this->Cities_model->get_details($city_options)->getResult();
        $cities_dropdown = array("" => "-");

        if ($cities) {
            foreach ($cities as $city) {
                $cities_dropdown[$city->id] = $city->city_name;
            }
        }
    ...
    protected function _get_maintask_clsifications_dropdown() {
        $maintask_cls_options = array("deleted" => "0");
        

        $maintask_cls = $this->Maintask_clsifications_model->get_details($maintask_cls_options)->getResult();
        $maintask_cls_dropdown = array("" => "-");

        if ($maintask_cls) {
            foreach ($maintask_cls as $maintask_clss) {
                $maintask_cls_dropdown[$maintask_clss->id] = $maintask_clss->title;
            }
        }

        return $maintask_cls_dropdown;
    }

    protected function _get_myclients_dropdown() {
        $options = array("deleted" => "0",'client_status'=> "1");
        if ($this->login_user->user_type == "staff") {
            if (!$this->can_manage_all_projects()) {
                $project_options["user_id"] = $this->login_user->id; //normal user's should be able to see only the projects where they are added as a team mmeber.
            }
    ...
    protected function _get_myclients_contacts_dropdown($client_id) {
        $options = array("deleted" => "0","client_id" => $client_id,"user_type" => "client");
        if ($this->login_user->user_type == "staff") {
            if (!$this->can_manage_all_projects()) {
                $project_options["user_id"] = $this->login_user->id; //normal user's should be able to see only the projects where they are added as a team mmeber.
            }
        ...
        if ($clients) {
            foreach ($clients as $client) {
                $clients_dropdown[$client->id] = $client->first_name;
            }
        }
    ...
    protected function can_show_tasks() {
        if ($this->login_user->user_type == "staff") {
            if ($this->can_manage_all_projects()) {
                return true;
            } else if (get_array_value($this->login_user->permissions, "can_show_tasks") == "1") {
                //check is user a project member
                return true;
            }
        } 
    ...
    protected function can_show_subtasks() {
        if ($this->login_user->user_type == "staff") {
            if ($this->can_manage_all_projects()) {
                return true;
            } else if (get_array_value($this->login_user->permissions, "can_show_subtasks") == "1") {
                //check is user a project member
                return true;
            }
        } 
    ...
    protected function can_update_maintask_after_closed() {
        if ($this->login_user->user_type == "staff") {
            if ($this->login_user->is_admin) {
                return true;
            } else if (get_array_value($this->login_user->permissions, "can_update_maintask_after_closed") == "1") {
                //check is user a project member
                return true;
            }
        } 
    ...
    protected function can_delete_subtasks() {
        if ($this->login_user->user_type == "staff") {
            if ($this->can_manage_all_projects()) {
                return true;
            } else if (get_array_value($this->login_user->permissions, "can_delete_subtasks") == "1") {
                //check is user a project member
                return true;
            }
        } 
    ...
    protected function can_add_suppliers() {
        if ($this->login_user->user_type == "staff") {
            if ($this->can_manage_all_projects()) {
                return true;
            } else if (get_array_value($this->login_user->permissions, "can_add_suppliers") == "1") {
                //check is user a project member
                return true;
            }
        } 
    ...
    protected function can_edit_suppliers() {
        if ($this->login_user->user_type == "staff") {
            if ($this->can_manage_all_projects()) {
                return true;
            } else if (get_array_value($this->login_user->permissions, "can_edit_suppliers") == "1") {
                //check is user a project member
                return true;
            }
        } 
    ...
    protected function can_delete_suppliers() {
        if ($this->login_user->user_type == "staff") {
            if ($this->can_manage_all_projects()) {
                return true;
            } else if (get_array_value($this->login_user->permissions, "can_delete_suppliers") == "1") {
                //check is user a project member
                return true;
            }
        } 
    ...
    protected function can_add_city() {
        if ($this->login_user->user_type == "staff") {
            if ($this->can_manage_all_projects()) {
                return true;
            } else if (get_array_value($this->login_user->permissions, "can_add_city") == "1") {
                //check is user a project member
                return true;
            }
        } 
    ...
    protected function can_edit_city() {
        if ($this->login_user->user_type == "staff") {
            if ($this->can_manage_all_projects()) {
                return true;
            } else if (get_array_value($this->login_user->permissions, "can_edit_city") == "1") {
                //check is user a project member
                return true;
            }
        } 
    ...
    protected function can_delete_city() {
        if ($this->login_user->user_type == "staff") {
            if ($this->can_manage_all_projects()) {
                return true;
            } else if (get_array_value($this->login_user->permissions, "can_delete_city") == "1") {
                //check is user a project member
                return true;
            }
        } 
    ...
    protected function supplier_permission() {
        if ($this->login_user->user_type == "staff") {
            if ($this->login_user->is_admin) {
                return true;
            } else if (get_array_value($this->login_user->permissions, "supplier_permission") == "1") {
                //check is user a project member
                return true;
            }
        } 
    ...
    protected function car_type_permission() {
        if ($this->login_user->user_type == "staff") {
            if ($this->login_user->is_admin) {
                return true;
            } else if (get_array_value($this->login_user->permissions, "car_type_permission") == "1") {
                //check is user a project member
                return true;
            }
        } 
    ...
    protected function drivers_permission() {
        if ($this->login_user->user_type == "staff") {
            if ($this->login_user->is_admin) {
                return true;
            } else if (get_array_value($this->login_user->permissions, "drivers_permission") == "1") {
                //check is user a project member
                return true;
            }
        } 
    ...
    protected function my_can_edit_tasks() {
        //if ($this->login_user->user_type == "staff") {
           if ($this->login_user->jon_title == "مبيعات") {
                //check is user a project member
                return true;
            }
       /* } else {
